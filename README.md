...
│   ├── separability_analysis.py  # ⑨ 类别可分性分析
│   └── k_selection_stability.py  # ⑩ K 值选择 + 稳定性分析
├── figures/                    # 运行生成的结果图
├── requirements.txt
└── README.md
```
...
### ⑨ 类别可分性分析 (`separability_analysis.py`)

为论证分割结果具有坚实的物理与统计基础（而非算法强行划分），对最终四类做可分性定量分析：绘制各类平均光谱曲线、计算类间欧氏距离矩阵与 Fisher 判别比（类间散度/类内散度）。
...
四类平均光谱呈清晰的阶梯式分离，**Fisher 判别比高达 11.0**（类间差异为类内离散的 11 倍），最小类间距离（90.5）显著大于最大类内标准差（26.2）。这表明四类地物在光谱空间中高度可分，K-means 的聚类结果具有可靠的统计依据。

![类别可分性分析](figures/separability.png)

![类别可分性分析](figures/separability.png)

### ⑩ K 值选择与稳定性分析 (`k_selection_stability.py`)

**K 值定量依据**：肘部法则显示 SSE 下降速率在 K=4 处明显放缓（肘部拐点）；轮廓系数在 K=2 最高（0.752，对应冰—水二分），K=4 仍达 0.736。K=2 虽指标略高但无法满足四类区分需求，K=4 兼顾分类精细度与聚类质量，为最优选择。

**稳定性**：以 10 个随机种子重复实验，轮廓系数均值 0.7356、**标准差仅 0.0001**，分割一致率达 **99.98%**，证明方法对随机初始化高度鲁棒、结果可复现。

![K 值选择与稳定性](figures/k_selection_stability.png)
